...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS sessions (
        ...
        status TEXT DEFAULT 'active', added_at TEXT DEFAULT CURRENT_TIMESTAMP,
        last_active TEXT
    )''')
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS redeemed_codes (
        ...
        user_id INTEGER, redeemed_at TEXT DEFAULT CURRENT_TIMESTAMP
    )''')
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not channel_sub or not group_sub:
        ...
        await update.message.reply_text("⚠️ يجب الاشتراك في القناة والمجموعة لاستخدام البوت!\n\nاشترك ثم اضغط 'تحقق من الاشتراك'", reply_markup=InlineKeyboardMarkup(keyboard))
        return
    keyboard = [
...
async def report_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.edit_text("📱 أرسل رقم الهاتف للتبليغ (مع مفتاح الدولة):\nمثال: +249123456789", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 إلغاء", callback_data="back")]]))
...
async def report_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.edit_text("👥 أرسل معرف/رابط المجموعة للتبليغ:\nمثال: @group أو https://t.me/group", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 إلغاء", callback_data="back")]]))
...
async def gift_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.edit_text("🎁 أرسل كود الهدية:", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 إلغاء", callback_data="back")]]))
...
<!DOCTYPE html>
<html dir="rtl">
<head><title>تسجيل دخول</title><meta charset="UTF-8">
<style>
...
button{width:100%;background:#e94560;color:white;padding:12px;border:none;border-radius:5px;cursor:pointer;font-size:16px;}
</style>
</head>
<body>
<div class="container">
...
def get_admin_html(users_count, sessions_count, reports_count, sessions_list, generated_code):
    gift_html = ""
    if generated_code:
    ...
    for s in sessions_list:
        ...
        <tr>
            ...
            <td>{s[1]}</td>
            <td><span style="padding:4px 12px;border-radius:20px;font-size:12px;color:white;{badge_class}">{status_text}</span></td>
            ...
            <td>
                <form method="post" action="/admin/delete_session" style="display:inline;">
                    ...
                    <button type="submit" style="background:#e94560;padding:8px 15px;border:none;border-radius:5px;color:white;cursor:pointer;">حذف</button>
                </form>
            </td>
        </tr>"""
...
<head>
    ...
    <style>
        *{{margin:0;padding:0;box-sizing:border-box;}}
        body{{font-family:'Segoe UI',Tahoma;background:#0f0f23;color:#e0e0e0;}}
        .sidebar{{width:250px;background:#1a1a3e;height:100vh;position:fixed;padding:20px;}}
        .sidebar h2{{color:#e94560;margin-bottom:30px;}}
        .sidebar a{{display:block;color:#aaa;text-decoration:none;padding:12px 15px;margin:5px 0;border-radius:8px;}}
...
<div class="main">
    ...
    <div class="stats" id="stats">
        <div class="stat-card"><h3>{users_count}</h3><p>👥 المستخدمين</p></div>
        <div class="stat-card"><h3>{sessions_count}</h3><p>📱 جلسات نشطة</p></div>
        <div class="stat-card"><h3>{reports_count}</h3><p>🚨 التبليغات</p></div>
    </div>
    ...
    <div class="card" id="sessions">
        ...
        <form method="post" action="/admin/add_session_web">
            ...
            <input type="text" name="code" placeholder="كود التفعيل">
            <button type="submit">✅ إضافة الجلسة</button>
        </form>
        <h3 style="margin-top:25px;color:#e94560;">الجلسات الحالية:</h3>
        <table>
            <tr><th>المعرف</th><th>الرقم</th><th>الحالة</th><th>التاريخ</th><th>حذف</th></tr>
            {sessions_html}
        </table>
    </div>
</div>
...
def admin_dashboard():
    ...
    return get_admin_html(users_count, sessions_count, reports_count, sessions_list, generated_code)
