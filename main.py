...
def redeem_gift_code(code, user_id):
    ...
    if gift[6] and datetime.now() > datetime.fromisoformat(gift[6]):
        conn.close()
...
def check_forced_subscription(user_id, context):
    try:
        ...
        chat_member2 = context.bot.get_chat_member(chat_id=GROUP_ID, user_id=user_id)
        ...
        conn = get_db()
        c = conn.cursor()
        c.execute("INSERT OR REPLACE INTO forced_subs (user_id, channel_sub, group_sub) VALUES (?, ?, ?)", 
                  (user_id, 1 if channel_sub else 0, 1 if group_sub else 0))
        conn.commit()
        conn.close()
...
async def admin_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"📋 التبليغات المعلقة: {pending_reports}",
        parse_mode='HTML'
    )
