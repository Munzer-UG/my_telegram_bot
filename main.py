...
@app.route('/')
def home():
    return "✅ البوت شغال 100%! تواصل مع البوت على تيليجرام."
...
def generate_referral_code(user_id):
    return f"REF_{user_id}_{''.join(random.choices(string.ascii_uppercase + string.digits, k=6))}"
...
def check_subscription(user_id, context):
    try:
        channel_member = context.bot.get_chat_member(chat_id=CHANNEL_LINK.split('/')[-1], user_id=user_id)
        group_member = context.bot.get_chat_member(chat_id=GROUP_LINK.split('/')[-1], user_id=user_id)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args and context.args[0].startswith('ref_'):
        ...
        if referrer_id != user.id:
            ...
            await update.message.reply_text(f"✅ تمت الدعوة بنجاح! حصل المدعو على نقطة إضافية.")
...
async def referral(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (f"🔗 رابط الدعوة الخاص بك:\n\n"
            f"{referral_link}\n\n"
            f"📌 كل صديق يسجل عبر رابطك تحصل على نقطة واحدة!\n"
...
async def report_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if points < 10:
        await query.message.edit_text(
            ...
            f"📱 تحتاج 10 نقاط لتبليغ رقم.\n\n"
            f"🔗 ادعُ أصدقاءك أو استخدم كود هدية للحصول على نقاط.",
    ...
    await query.message.edit_text(
        "📱 أرسل الرقم المراد تبليغه:\n"
        "مثال: 966501234567\n\n"
        "⚠️ سيتم خصم 10 نقاط من رصيدك.",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="back")]])
    )
...
async def report_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.edit_text(
        "👥 أرسل معرف المجموعة أو الرابط:\n"
        "مثال: @group_name\n\n"
        "⚠️ سيتم خصم 15 نقطة من رصيدك.",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="back")]])
    )
...
async def handle_report_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # تنفيذ التبليغ عبر الجلسات النشطة
    ...
    if success_count == 0:
        await update.message.reply_text(
            f"❌ فشل التبليغ!\n"
            f"📌 {message}\n\n"
            f"📞 تواصل مع المطور لإضافة جلسات.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="back")]])
        )
        return
    ...
    await update.message.reply_text(
        ...
        f"📊 {message}\n"
        f"⭐ تم خصم {points_cost} نقطة من رصيدك.",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data="back")]])
    )
...
def admin():
    if request.method == 'POST':
        ...
        if action == 'add_session':
            ...
            conn.commit()
            conn.close()
        elif action == 'set_report_settings':
            report_type = request.form['report_type']
            points_cost = int(request.form['points_cost'])
            conn = sqlite3.connect('bot.db')
            c = conn.cursor()
            c.execute("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)", 
                      (f"report_{report_type}_cost", points_cost))
            conn.commit()
            conn.close()
